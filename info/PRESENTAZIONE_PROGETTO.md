...
**Status:** Produzione / Docker Compose Orchestrated (v2.0.0 — binding sessione FastAPI, campi utente inglese) 

---

## 1. SOMMARIO ESECUTIVO

Il sistema è un'**architettura ibrida Monolite Modulare + Microservizi** che integra una web app **Flask** (autenticazione, gruppi, profilo utente con nuovi campi `tax_id_code`, `full_name`, `date_of_birth`, `gender`, `birth_city_country`) con un microservizio **FastAPI** per l'ingestione asincrona della telemetria GPS, autenticato tramite cookie di sessione (`/auth/check_session`).
...
**Nota tecnica (autenticazione condivisa):** L'endpoint `/auth/check_session` verifica il cookie `session` Flask. FastAPI (`verify_flask_session`) chiama questo endpoint e sovrascrive `user_id` nel payload (`binding`) con `session["user_id"]`, impedendo che un utente autenticato invii dati per un altro utente (`impersonation`).
